...
{
    /// <summary>
    /// Pagina per ottenere l'input della ricetta per il Pane.
    /// </summary>
    public sealed partial class Pane : Page
    {
        ...
        private void Calcola_Click(object sender , RoutedEventArgs e)
        {
            ...
            double _idroP, _tAmbiente, _lievEsterna, _lievFrigo, _sale;
            ...
            try //Evita errori in input
            {
                ...
                double peso_lievito     = 0.00226 * F_Temperatura(_tAmbiente) * peso_impasto * F_idro(_idroP) * (1 + 0.006 * _sale) / ((_lievEsterna + _lievFrigo > 3 ? _lievEsterna + _lievFrigo : 3) - 0.90 * _lievFrigo - 1.26);

                MainFrame.Content = new Risultato( peso_olio , peso_sale , peso_acqua , peso_farina , peso_lievito , Risultato.Impasto.Pane );
            }
            catch (Exception)
            {
